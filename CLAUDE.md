...
- **Renforcement sécurité** (voir `schema_securite_renforcee.sql`) suite à une revue complète de l'application — déploiement bloqué une nouvelle fois (erreur générique "Deployment failed, try again later" puis blocage "Queued", résolu en supprimant/recréant l'environnement `github-pages`) :
