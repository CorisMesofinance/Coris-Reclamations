...
- **Assignation par liste déroulante + notification email** : le champ "Assigné à" de la modale staff est maintenant une liste déroulante peuplée depuis `profiles` (actifs uniquement, voir `schema_assignation.sql` pour la policy RLS requise). Quand l'assignation change vers une personne ayant un email connu, `index.html` appelle l'Edge Function `supabase/functions/notifier-assignation` (Resend) qui lui envoie un email. **Reste à faire par l'utilisateur** : créer le compte Resend + clé API, déployer l'Edge Function et son secret `RESEND_API_KEY` sur Supabase (aucun accès réseau direct depuis Claude Code pour le faire).
...
- **Pas encore fait** : pièces jointes (Storage), commentaires dédiés sur les réclamations, QR code, notifications email à la création/changement de statut (seule l'assignation déclenche un email pour l'instant), SMS, relances pg_cron.
