...
- **Assignation par liste déroulante + notifications email (assignation + nouvelle réclamation)** : le champ "Assigné à" de la modale staff est une liste déroulante peuplée depuis `profiles` (actifs uniquement, voir `schema_assignation.sql`). Deux événements envoient un email **directement côté client via EmailJS** (`EMAILJS_PUBLIC_KEY` / `EMAILJS_SERVICE_ID` / `EMAILJS_TEMPLATE_ASSIGNATION`) :
  - Assignation à une personne ayant un email connu → email à cette personne (`notifierAssignation`)
  - Dépôt d'une nouvelle réclamation par un client → email aux 3 admins (`notifierNouvelleReclamation`, liste codée en dur dans `ADMIN_EMAILS` puisque le formulaire public, non authentifié, ne peut pas lire `profiles`)
  - **Un seul template EmailJS partagé** (`template_j4c3ihr`) pour les deux cas — le compte EmailJS gratuit limite le nombre de templates, donc le contenu a été rendu générique via 3 variables fournies dynamiquement par le JS : `objet` (sujet), `salutation`, `intro` (accroche), en plus de `reference`/`agence`/`type_plainte`/`description` communs.
...
- **Pièces jointes livrées** (voir `schema_pieces_jointes.sql`) : le client peut joindre jusqu'à 5 fichiers (images/PDF/vidéos, 10 Mo max chacun) en déposant sa réclamation. Stockage réel dans le bucket Storage privé `pieces-jointes` (contrairement aux autres apps Coris, ici on garde le fichier — voir note en tête de ce document). Upload direct par anon (`uploaderFichier()`), puis `soumettre_reclamation` (RPC étendue avec le paramètre `p_pieces_jointes`) crée les lignes `pieces_jointes_reclamations` dans la même transaction que la réclamation, en connaissant l'id généré côté serveur — évite d'exposer l'id ou d'ouvrir une policy INSERT à anon sur cette table. Côté staff, la modale de détail affiche la liste et génère une URL signée (60s) à la demande (`voirPieceJointe()`) plutôt que de rendre le bucket public.
- **Notifications email** : assignation et nouvelle réclamation (vers les 3 admins) déclenchent un email — voir plus haut. **Reste à faire** : notification à chaque changement de statut (pas seulement assignation/création), commentaires dédiés (indépendants de l'historique de statut), relances automatiques pg_cron au dépassement du seuil rouge SLA.
- **SMS abandonné** — décision explicite de l'utilisateur, ne pas proposer de le construire sauf demande explicite future.
- **Renforcement sécurité** (voir `schema_securite_renforcee.sql`) suite à une revue complète de l'application — déploiement resté bloqué un moment (alternance entre erreur générique "Deployment failed, try again later" et blocage "Queued", persistant même après suppression/recréation de l'environnement `github-pages` ; nouvelle tentative après une pause) :
  - RLS + trigger : un compte désactivé (`actif=false`) perd immédiatement le droit de modifier une réclamation (avant, il gardait l'accès jusqu'à expiration de son jeton, ~1h).
  - Bucket Storage `pieces-jointes` : limite serveur ajoutée (10 Mo, images/PDF/vidéo uniquement) — avant, seule la validation côté client existait, contournable en appelant l'API directement.
  - `soumettre_reclamation` : garde-fou anti-flood (max 20 réclamations/2 min, limite globale grossière) pour éviter qu'un spam n'épuise le quota mensuel gratuit EmailJS et ne coupe silencieusement les vraies notifications.
  - Côté `index.html` : rafraîchissement automatique du jeton de session (`sbFetch()`/`rafraichirSession()` — avant, la session expirait silencieusement après ~1h avec des erreurs HTTP 401 génériques) et verrouillage après 15 min d'inactivité (`startInactivityWatcher()`, même pattern que Coris-Point-Hebdo) — absent jusqu'ici alors que l'appli stocke des pièces jointes clients potentiellement sensibles.
  - En-tête staff : corrigé le chevauchement sur très petit écran (media query `max-width:700px`).
  - **Non traité, à surveiller plus tard si besoin** : pas de pagination (au-delà de la limite par défaut de l'API PostgREST, les réclamations les plus anciennes cesseraient de charger — pas un problème au volume actuel) ; pas de "mot de passe oublié" en libre-service pour le staff ; petite fenêtre de risque de doublon de référence en cas de double soumission strictement simultanée (protégé par la contrainte UNIQUE, mais message d'erreur générique côté client).
- **Workflow de clôture et verrouillage** (voir `schema_workflow_cloture.sql`) :
  - Niveau d'urgence : fixé par le client à la création, verrouillé pour tout le monde (staff comme admin) définitivement — select toujours `disabled` côté UI, et le trigger `verifier_permission_maj_reclamation` rejette toute tentative de changement côté base.
  - Statut "Clos" : réservé aux admins actifs — l'option est masquée du `<select>` pour les non-admins, et le trigger la bloque aussi côté base si jamais contournée.
  - Réclamation "Clos" : devient totalement immuable, pour tout le monde y compris les admins (le trigger rejette toute UPDATE dès que `OLD.statut = 'clos'`) ; côté UI, tous les champs sont désactivés et le bouton "Enregistrer" est masqué.
  - Commentaire : obligatoire pour un non-admin qui enregistre (validation côté JS avant l'appel réseau), resté facultatif pour les admins.
  - Rappel du projet Supabase auquel se rattache tout ceci : `pqsczevlnvdayelkjtmq` (plan gratuit) — celui-ci se met en pause après 7 jours sans activité API (pas de lien avec l'usage du dashboard Supabase.com en tant que tel) ; un `Restore` manuel depuis le dashboard suffit à le réactiver, sans perte de données. Passer sur le plan payant Pro supprime cette mise en pause si le volume d'usage le justifie.
